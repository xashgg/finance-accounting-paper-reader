...
Create a rigorous note for one identified paper. Resolve the vault in this order: the user's explicit destination, established session configuration, the process environment variable `OBSIDIAN_VAULT_PATH`, then `OBSIDIAN_VAULT_PATH` in a `.env` file beside this `SKILL.md`. Read `.env` as plain configuration data; accept a single unquoted or matching single/double-quoted value, ignore blank lines and comments, and never execute or source the file. If no vault is configured, ask for its location before saving; reading and analysis may proceed independently. Canonical output: `<vault>/10_Papers/<citekey>.md`. Expand environment variables and `~` where supported, resolve the result to an absolute host path, and verify that the vault directory exists before using it. Never guess a personal folder path or substitute a similarly named vault silently.
